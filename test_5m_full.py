#!/usr/bin/env python3
"""Test: everything on 5m bars (volume bars + ATR + indicators)."""
...
def prepare(tf="15m"):
    raw = raw_15m if tf == "15m" else raw_5m
    data = {}
    for sym in config.TRADING_PAIRS:
        ...
        data[sym] = vdf
        console.print(f"  {sym}: {len(vdf)} vol bars")
    return data
...
console.print("[bold]15m (current):[/bold]")
d15 = prepare("15m")
console.print(f"\n[bold]5m (full):[/bold]")
d5 = prepare("5m")
...
console.print(f"\nRunning 15m...")
r15 = run_test(d15)
console.print(f"Running 5m...")
r5 = run_test(d5)

table = Table(title="15m vs 5m Full (192d)", box=box.ROUNDED)
table.add_column("TF", style="bold")
table.add_column("Vol bars", justify="right")
table.add_column("Trades", justify="right")
table.add_column("WR", justify="right")
table.add_column("NET", justify="right")
table.add_column("TP", justify="right")
table.add_column("SL", justify="right")
vb15 = min(len(d) for d in d15.values())
vb5 = min(len(d) for d in d5.values())
table.add_row("15m (current)", str(vb15), str(r15["trades"]), f"{r15['wr']:.1f}%", f"${r15['net']:+.0f}", str(r15["tp"]), str(r15["sl"]))
table.add_row("5m (full)", str(vb5), str(r5["trades"]), f"{r5['wr']:.1f}%", f"${r5['net']:+.0f}", str(r5["tp"]), str(r5["sl"]))
console.print(table)
