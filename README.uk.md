...
| Фільтр | Що робить |
|--------|-----------|
| **Volume** | Блокує при об'ємі < 80% від середнього |
| **ADX** | Блокує якщо ADX < 15 (немає тренду) |
| **EMA Тренд** | Блокує LONG при EMA9 < EMA21 (ведмежий), SHORT при EMA9 > EMA21 (бичачий) |
| **Order Book** | Блокує проти тиску стакану (>0.5 дисбаланс) |
| **NYSE Protection** | Закриває угоди + блокує нові сигнали під час відкриття NYSE (13:00–15:00 UTC) |
...
- SL/TP по 5m ATR (SL = 1.5x ATR, TP = 2.5x ATR → R:R = 1:1.67)
- **Мінімальний SL floor** — стоп не менше 0.3% від ціни входу
...
- **Реалістичне виконання**: вхід по наступній свічці, комісії (0.08%), проскальзування (0.01%)
- **Trailing stop + early exit** активні в бектесті
- **Всі фільтри включені** (volume, ADX, EMA тренд, dead hours)
- **Dead hour protection**: закриває угоди + блокує нові під час NYSE open
- **Результат**: ~+95% net PnL за ~10 днів (Win Rate 67%, Sharpe 17.4, Profit Factor 1.84)
